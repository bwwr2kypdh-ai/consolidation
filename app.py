...
def calc_settlement_oedometer(layer, delta_sigma):
    ...
    if fs <= sig_c: 
        return H * (cr / (1 + e0)) * math.log10(fs / sig_0)
    else: 
        return (H * (cr / (1 + e0)) * math.log10(sig_c / sig_0)) + (H * (cc / (1 + e0)) * math.log10(fs / sig_c))
...
with st.sidebar.expander(tr("💾 Sauvegarder / Charger Projet", "💾 Save / Load Project")):
    ...
    if uploaded_file is not None:
        try:
            ...
            if st.session_state['project_data']:
                # Reconstruire les dataframes avec sécurité
    ...
    if st.session_state['project_data']:
        ...
        st.download_button(tr("📥 Exporter Projet", "📥 Export"), data=json.dumps({'project_data': out}), file_name="master_project.json", mime="application/json")
...
with col_zones:
    ...
    if not drawn_polygons: st.info(tr("Dessinez des zones sur la carte.", "Draw polygons on map."))
...
if btn_api:
    with st.spinner(tr("Extraction MNT par lots sécurisés...", "Fetching DEM...")):
        if "CSV" in api_choice and uploaded_mnt:
            ...
            if all_pts:
                elevs = []
                chunk_size = 50 # Sécurité anti-crash
                ...
                if "Google" in api_choice:
                    ...
                    for i in range(0, len(all_pts), chunk_size):
                        chunk = all_pts[i:i+chunk_size]
                        lats_str, lons_str = ",".join([str(p[0]) for p in chunk]), ",".join([str(p[1]) for p in chunk])
                        try:
                ...
                if elevs and len(elevs) == len(all_pts):
                    ...
                    st.success(tr("Topographie enregistrée ! Cliquez sur Calculer.", "DEM Saved !"))
                else: st.error("Échec API : Données incomplètes.")
...
if btn_calc or (btn_api and st.session_state['cached_mnt'] is not None):
    if not zones_params or st.session_state['cached_mnt'] is None:
        ...
        with st.spinner(tr("Analyse itérative...", "Iterating analysis...")):
            ...
            st.success(tr("Calculs Itératifs OK.", "Iterative calculations OK."))
    ...
    with tabs[1]:
        ...
        for idx, row in d['results'].iterrows():
            with st.container():
                ...
                # SÉCURITÉ JSON LÉGACY POUR ÉVITER TOUT KEYERROR
                s_oedo = row.get('S_oedo', row.get('S_max', 0.0))
                s_cpt = row.get('S_cpt', row.get('S_max', 0.0))
                s_spt = row.get('S_spt', row.get('S_max', 0.0))
                fs_val = row.get('FS', row.get('FS_Mudwave', 999.0))
                
                c_1, c_2 = st.columns(2)
                with c_1:
                    fig_c = go.Figure(data=[go.Bar(name='Oedo', x=['Méthodes'], y=[s_oedo]), go.Bar(name='CPTu', x=['Méthodes'], y=[s_cpt]), go.Bar(name='SPT', x=['Méthodes'], y=[s_spt])])
                ...
                with c_2:
                    ...
                    if fs_val < 1.3:
                        ...
                        st.info(f"💡 Conseil : Levée maximale sécuritaire de {(5.14*su_zone)/(gamma_fill*1.3):.2f}m.")
        ...
        if not d['pvds'].empty:
            ...
            for z in d['zones']:
                ...
                if len(df_zone_pvd) > 2000:
                    vis_sp = z.get('spacing', 1.2) * math.sqrt(len(df_zone_pvd) / 2000.0)
                    df_vis = generate_pvd_grid(z['coords'], vis_sp)
                else: df_vis = df_zone_pvd
                if not df_vis.empty:
                    fig_pvd.add_trace(go.Scattermapbox(lat=df_vis['Lat'], lon=df_vis['Lon'], mode='markers', marker=dict(size=3, color='blue', opacity=0.7), name=f"PVD Zone {z['id']}"))
