...
def check_password():
    ...
    if not st.session_state.get("authenticated", False):
        st.title("🔒 Accès Sécurisé Master SIG / Secure Access")
...
def calculate_loads(dead_load, live_load, surcharge_ratio, gamma_fill):
    ...
    return final_load, final_load * surcharge_ratio
...
with st.sidebar.expander(tr("💾 Sauvegarder / Charger Projet", "💾 Save / Load Project")):
    ...
    if st.session_state['project_data']:
        ...
        st.download_button(tr("📥 Exporter le Projet", "📥 Export Project"), data=json.dumps({'project_data': out}), file_name="projet_sig_master.json")
...
# --- RÉINTÉGRATION DE TOUS LES PARAMÈTRES PAR ZONE ---
zones_params = []
with col_zones:
    ...
    for i, poly in enumerate(drawn_polygons):
        with st.expander(f"🔴 Zone {i+1}", expanded=(i==0)):
            ...
            with c_ed:
                ...
                s0 = st.number_input("σ'0", value=40.0, key=f"s0_{i}")
                sc = st.number_input("σ'c", value=45.0, key=f"sc_{i}")
            with c_insitu:
                st.caption("In-Situ / Risque")
...
if btn_api:
    with st.spinner(tr("Extraction MNT...", "Fetching DEM...")):
        if "CSV" in api_choice and uploaded_mnt:
            ...
            # Génération d'un nuage de points strictement interne aux zones
            res_m = 15.0 / 111000.0
            all_pts = []
            ...
            if all_pts:
                # Simulation d'appel (Open-Meteo pour l'exemple)
                # Note: Pour un vrai projet, insérer ici la boucle chunking Google API
                elevs = [2.0 + math.sin(p[0]*1000) for p in all_pts] 
    ...
    if not zones_params or st.session_state['cached_mnt'] is None:
        ...
        with st.spinner(tr("Analyse itérative...", "Iterating analysis...")):
            ...
            for z in zones_params:
                ...
                q_total_def = q_remblai + dead_load_def + live_load_def
                q_travaux = q_total_def * surcharge_ratio
                
                # Tassements (Tri-méthode consolidé)
                ...
                # Logistique Compensée
                h_terre_totale = max(0, delta_z) + s_max
                vol_tot = area * h_terre_totale
                
                # Stabilité
                fs = (5.14 * z['Su']) / (gamma_fill * h_terre_totale) if h_terre_totale > 0 else 999
                ...
                results.append({
                    ...
                    'Area': area, 'H_fill': h_terre_totale, 'Z_nat': z_nat, 'S_oedo': s1, 'S_cpt': s2, 'S_spt': s3
                })
                
            st.session_state['project_data'] = {
                'results': pd.DataFrame(results), 'pvds': all_pvds, 'mnt': mnt, 'zones': zones_params
            }
...
if st.session_state['project_data']:
    ...
    # --- TAB 2: RISQUES & TASSEMENTS ---
    with tabs[1]:
        st.subheader("Analyse Comparative et Stabilité")
            ...
            with st.container():
                ...
                with c_1:
                    fig_c = go.Figure(data=[
                        go.Bar(name='Oedomètre', x=['Méthodes'], y=[row['S_oedo']]),
                        go.Bar(name='CPTu', x=['Méthodes'], y=[row['S_cpt']]),
                        go.Bar(name='SPT', x=['Méthodes'], y=[row['S_spt']])
                    ])
                    fig_c.update_layout(height=300, title="Comparaison des Modèles"); st.plotly_chart(fig_c, use_container_width=True)
                with c_2:
                    if row['FS'] < 1.3:
    ...
    with tabs[2]:
        ...
        # Anti-Moiré dynamic downsampling
    ...
    with tabs[3]:
        ...
        with c_m1:
            ...
            st.plotly_chart(f_cp, use_container_width=True)
        ...
        with c_m2:
            ...
            f_suivi = go.Figure()
            f_suivi.add_trace(go.Scatter(x=days, y=S_th, name='Théorique (Design)', line=dict(dash='dash', color='blue')))
            f_suivi.add_trace(go.Scatter(x=df_mon['Jour'], y=df_mon['Tassement_m'], name='Terrain (Plaque)', mode='markers+lines', marker=dict(color='red', size=10)))
